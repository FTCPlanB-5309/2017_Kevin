...
public class ColorSensorBackward {
    RobotHardware robot;
    Telemetry telemetry;
    Gyro gyro = new Gyro(robot, telemetry);

    public ColorSensorBackward(RobotHardware robot, Telemetry telemetry) {
        this.robot = robot;
    ...
    public void findColumn (int leftOffset, int centerOffset, int rightOffset, RelicRecoveryVuMark column) {
        ...
        while (robot.leftWheel.isBusy() && robot.FloorColorSensor.red() < 1) {
