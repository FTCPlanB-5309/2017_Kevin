...
public class RobotHardware {
    ...
    public void backward(double power, int distance)
    {
        ...
        while(leftWheel.getCurrentPosition() != 0 || rightWheel.getCurrentPosition() != 0)
            Thread.yield();
        int newLeftTarget =  (int)(distance * COUNTS_PER_INCH);
        int newRightTarget = (int)(distance * COUNTS_PER_INCH);
    ...
    public void leftWheelTurn(double power, int degrees){
    ...
    public void rightWheelTurn(double power, int degrees){
        int clicks = degrees*CLICKS_PER_DEGREE;
    ...
    public void findLine(double power){
        ...
        leftWheel.setPower(power);
        rightWheel.setPower(power);
        while (OpticalSensor.getLightDetected() < .3) {
            Thread.yield();
        }
        // Stop all motion;
        leftWheel.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
    ...
    public void poke(int alliance, double power) throws InterruptedException {
    ...
    public void shooterSpeed(){
        if(SToggle) {
            ...
            leftShooter.setMaxSpeed(SHOOTER_SPEED);
            rightShooter.setMaxSpeed(SHOOTER_SPEED);*/
            leftShooter.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
            rightShooter.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
            leftShooter.setPower(0.675);
            rightShooter.setPower(0.675);
            SToggle = false;
        }
        else
        {
        ...
        long stopTime;
        long currentTime;
        stopTime = System.currentTimeMillis() + 100;
